...
function accessControl (options) {
  options = options || {};
  var ignore = utils.merge(exports.ignore, options.ignore || [])
    , secure = utils.merge(exports.secure, options.secure || [])
    , requiredLoggedIn = ('undefined' == typeof options.requiredLoggedIn) ? exports.requiredLoggedIn : options.requiredLoggedIn
    , requiredRoles = utils.merge(exports.requiredRoles, options.requiredRoles || [])
  ...
  return function (req, res, next) {
    // Ignore path ?
    if (utils.match(req.pathname, ignore)) return next();
    // Populate req.user
    retrieve(req, function (loggedin, roles) {
      ...
      // Protect path ?
      if (req.user && utils.match(req.pathname, secure)) {
